...
def main
  puts "Calculadora de Analisis de Riesgo Clientes Nuevos"
  ...
  begin
    ...
    prob = probabilidades.sum

    puts "LA PROBABILIDAD TOTAL DE QUEDAR DESEMPLEADO EN #{variableJS[:tiempo_a_calcular]} MESES es del #{probabilidades.sum}%"

    if prob > 40
        puts "El cliente debe ser RECHAZADO"
        elsif prob>=20 && prob<=40
            puts "El cliente debe ser REVISADO MANUALMENTE"
        else
            puts "El cliente debe ser APROBADO"
    end
